...
- 较长扫描可以随时取消，已经串流出来的候选股会保留。
- 同一批评分结果可以同时查看个股和板块分析。
- 可以在本地保存最近扫描，并导出 Markdown 或 JSON 研究笔记。
...
- 通过浏览器 `AbortController` 支持取消扫描请求。
- 共享短期内存 TTL cache，降低重复行情和新闻请求。
- 本地保存扫描记录，并支持 Markdown 和 JSON 导出，方便后续研究。
